...
#define _DIST_MAX 500 // [1352] 측정 거리의 최댓값을 410mm로 설정
...
// Servo range
#define _DUTY_MIN 1250    // //[2998] Servo Minimum microseconds
#define _DUTY_NEU 1550   //[2983] Servo 중간값 설정
#define _DUTY_MAX 1850       // [2979] Servo Max값 설정


// Servo speed control
#define _SERVO_ANGLE 40 // [2992] 서보 각 설정
#define _SERVO_SPEED 800 // [2976] 서보 스피드 설정
#define _RAMPUP_TIME 30
// Event periods
#define _INTERVAL_DIST 10   // [2987] 거리측정 INTERVAL값 설정
#define _INTERVAL_SERVO 23  // [2980] 서보 INTERVAL값 설정
...
double last_error;
// Servo speed control
int duty_chg_per_interval, duty_chg_max ; // current speed (unit: us/interval)
...
void setup() {
  ...
  duty_chg_max = (_DUTY_MAX - _DUTY_MIN) * ((float)_SERVO_SPEED / 180) * ((float)_INTERVAL_DIST / 1000); //[2985] duty_chg_per_interval 설정
  duty_chg_adjust = (float) duty_chg_max * _INTERVAL_DIST / _RAMPUP_TIME;
...
void loop() {
  ...
  if (event_dist) {
    ...
    if (duty_target > _DUTY_MAX) {
      duty_target = _DUTY_MAX; //[1352] 조건일때 duty_target값을 _DUTY_MAX으로
    }
    else if (duty_target < _DUTY_MIN) {
      duty_target = _DUTY_MIN; //[1352] 조건일때 duty_target값을 _DUTY_MIN으로
    }
