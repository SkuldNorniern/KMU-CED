...
#define _DUTY_NEU 1444 // servo neutral position (90 degree)
...
int k;
float scale, dist_ema, alpha; // used for pulse duration to distance conversion
Servo myservo;
...
// get a distance reading from the USS
  ...
  if(k<1)k++,dist_ema=dist_raw;
  else if(k==1)dist_ema=alpha*dist_raw+(1-alpha)*dist_ema;


  dist_ema=round(dist_ema);
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) reading = dist_prev; // return 0 when out of range.
